...
class InputParsingStage:
    ...
    def __init__(self) -> None:
        """Initialize parser warning state."""
    ...
    def run(self, inputs: list[PipelineInput]) -> list[CandidateRecord]:
        ...
        for pipeline_input in inputs:
            ...
            except (ParserError, GitHubAPIError) as exc:
                if not isinstance(pipeline_input.parser, GitHubProfileURLParser):
                    raise
                warning = f"GitHub enrichment skipped for {pipeline_input.source_path}: {exc}"
                self.warnings.append(warning)
                logger.warning(
                    "GitHub input skipped",
                    extra={"source_path": str(pipeline_input.source_path), "error": str(exc)},
                )
        return records
        ...
        for record in records:
            for link in record.links:
                ...
                try:
                    ...
                    warning = f"GitHub enrichment skipped for {classification.url}: {exc}"
                    self.warnings.append(warning)
                    logger.warning(
...
class CandidatePipeline:
    ...
    def run(
        ...
        *,
        ...
        github_warnings = list(getattr(self._input_stage, "warnings", []))
        enriched_records = self._github_enrichment_stage.run(parsed_records)
        github_warnings.extend(getattr(self._github_enrichment_stage, "warnings", []))
