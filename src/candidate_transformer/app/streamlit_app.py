...
def _render_results(result: PipelineResult) -> None:
    ...
    github_warnings = getattr(result, "github_warnings", [])
    if github_warnings:
        st.warning("GitHub enrichment was skipped for one or more profiles. Continuing with available candidate data.")
        st.dataframe(
            [{"Warning": warning} for warning in github_warnings],
            use_container_width=True,
            hide_index=True,
        )
